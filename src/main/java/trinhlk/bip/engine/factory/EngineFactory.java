 ...
 */
package trinhlk.bip.engine.factory;

import trinhlk.bip.core.api.BIPEngine;
import trinhlk.bip.core.api.BIPGlue;
import trinhlk.bip.engine.core.BDDBIPEngineImpl;
import trinhlk.bip.engine.core.BehaviourEncoderImpl;
import trinhlk.bip.engine.core.CurrentStateEncoderImpl;
import trinhlk.bip.engine.core.DataEncoderImpl;
import trinhlk.bip.engine.core.GlueEncoderImpl;
import trinhlk.bip.engine.api.BDDBIPEngine;
import trinhlk.bip.engine.api.BIPCoordinator;
import trinhlk.bip.engine.api.BehaviourEncoder;
import trinhlk.bip.engine.api.CurrentStateEncoder;
import trinhlk.bip.engine.api.DataEncoder;
import trinhlk.bip.engine.api.GlueEncoder;
import trinhlk.bip.engine.coordinator.BIPCoordinatorImpl;
...
public class EngineFactory {
	...
	public BIPEngine create(String id, BIPGlue glue) {
		...
		if (glue.getDataWires().size() == 0) {
			...
			bipEngine = new trinhlk.bip.engine.coordinator.DataCoordinatorKernel(basicCoordinator, dataEncoder);
		}
		...
		((BIPCoordinator) actor).initialize();
