...
import trinhlk.bip.core.annotations.Guard;
import trinhlk.bip.core.annotations.Port;
import trinhlk.bip.core.annotations.Ports;
import trinhlk.bip.core.annotations.Transition;
import trinhlk.bip.core.api.Executor;
...
@Ports({ @Port(name = "port_end", type = PortType.spontaneous),
		 @Port(name = "port_on", type = PortType.enforceable), 
		 @Port(name = "off", type = PortType.enforceable), 
 @Port(name = "finished", type = PortType.enforceable),
		@Port(name = "internal", type = PortType.internal) })
@ComponentType(initial = "location_off", name = "trinhlk.bip.spec.SwitchableRouteForBIPBuilding")
public class SwitchableRouteForBIPBuilding implements CamelContextAware, InitializingBean, DisposableBean {
	...
	public SwitchableRouteForBIPBuilding(String routeId) {
	 ...
	 */
	@Transition(name = "off", source = "location_on", target = "wait", guard = "")
	...
	@Transition(name = "port_end", source = "wait", target = "done", guard = "!isFinished")
	public void spontaneousEnd() throws Exception {
		logger.info("Received end notification for the route {}.", routeId);
	}

	@Transition(name = "internal", source = "wait", target = "done", guard = "isFinished")
	...
	@Transition(name = "finished", source = "done", target = "location_off", guard = "")
	public void finishedTransition() throws Exception {
		logger.debug("Transitioning to off state from done for {}.", routeId);
	}

	@Transition(name = "port_on", source = "location_off", target = "location_on", guard = "")
